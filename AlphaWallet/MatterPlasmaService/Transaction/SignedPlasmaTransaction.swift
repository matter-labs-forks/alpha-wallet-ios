...
public struct SignedPlasmaTransaction {

    private let helpers = PlasmaTransactionHelpers()
    ...
    public func recoverSender() throws -> EthereumAddress {
        guard let hash = try? PlasmaTransactionHelpers.hashForSignature(data: self.transaction.data) else {throw PlasmaErrors.StructureErrors.wrongData}
        var v = self.v
        if v > 3 {
            v -= BigUInt(27)
        }
        ...
        guard let addressData = try? PlasmaTransactionHelpers.publicToAddressData(signerPubKey) else {throw PlasmaErrors.StructureErrors.wrongAddress}
        guard let address = EthereumAddress(addressData) else {throw PlasmaErrors.StructureErrors.wrongAddress}
        return address
    }
    
    private func marshalSignature(v: Data, r: Data, s: Data) -> Data? {
...
extension SignedPlasmaTransaction: Equatable {
    public static func ==(lhs: SignedPlasmaTransaction, rhs: SignedPlasmaTransaction) -> Bool {
